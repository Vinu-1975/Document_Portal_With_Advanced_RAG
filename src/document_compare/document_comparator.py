...
class DocumentComparatorLLM:
    ...
    def compare_documents(self, combined_docs: str):
        """
        Compares the two documents and returns the comparison result
        """
        try:
            inputs = {
    ...
    def _format_response(self, response_parsed: list[dict]) -> pd.DataFrame:
        """
        Formats the response from the model in a structured manner
        """
        try:
            ...
            raise DocumentPortalException("Error in _format_response", sys)
