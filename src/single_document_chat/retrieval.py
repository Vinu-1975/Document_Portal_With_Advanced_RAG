...
class ConversationalRAG:
    def __init__(self, session_id: str, retriever):
        try:
            ...
            self.log.info(
                "Created RunnableWithMessageHistory", session_id=self.session_id
            )

        except Exception as e:
            self.log.error(
                f"Error in ConversationalRAG __init__",
                error=str(e),
    ...
    def _load_llm(self):
        try:
            ...
            self.log.info("Loaded LLM", session_id=self.session_id)
            return llm

        except Exception as e:
            self.log.error(f"Error in _load_llm", error=str(e))
    ...
    def _get_session_history(self, session_id: str):
        try:
            ...
            self.log.error(
                f"Error in _get_session_history", error=str(e), session_id=session_id
            )

            raise DocumentPortalException("Error in _get_session_history", sys)
        ...
        try:
            ...
            if not os.path.isdir(index_path):
                raise FileNotFoundError(
                    f"FAISS index directory not found in {index_path}"
                )
            vectorstore = FAISS.load_local(index_path, embeddings)
            self.log.info("Loaded vectorstore from FAISS index", index_path=index_path)
            return vectorstore.as_retriever(
                search_type="similarity", search_kwargs={"k": 5}
            )

        except Exception as e:
            self.log.error(f"Error in load_retriever_from_faiss", error=str(e))
    ...
    def invoke(self, user_input: str):
        try:
            ...
            self.log.error(f"Error in invoke", error=str(e), session_id=self.session_id)
